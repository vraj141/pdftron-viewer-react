declare module collab-server
{
	...
	import { TestResolversErrorMessages } from './testResolvers';
	import type { UserResolvers } from '../../resolvers';
	const _default: (resolvers: UserResolvers) => Promise<TestResolversErrorMessages>;
	/**
	 * Test the resolvers reading or writing correct data
	 * @param {object} resolvers The object of the resolver
	 * @returns {array} Returns error messages
	 */
	export default _default;

	import type { UserResolvers } from '../../resolvers';
	import { TestData } from '../../util/generator';
	type TestCall = {
	    params: string | Record<any, any>;
	    ensureNotNull: boolean;
	    skipTest?: boolean;
	    /**
	     * @param result fetched data returned by the user resolver
	     * @param resolver the resolver passed in by user
	     * @param params the params that were passed into the resolver
	     */
	    test: (result: any, resolver: string, params: any) => void;
	};
	...
	type ErrorMessage = {
	    ...
	    queryParams?: any;
	    error: {
	        type: 'data_type_invalid' | 'data_missing' | 'data_invalid' | 'resolver_missing';
	        property?: string;
	        expected?: any;
	        received?: any;
	        message: string;
	...
	export type TestResolversErrorMessages = ErrorMessage[];
	export const errorMessages: TestResolversErrorMessages;
	export const resetErrorMessages: () => number;
	export const compareResult: (value: any, other: any, resolver: string, queryParams: any) => boolean;
	export const assert: (value: any, expectedValue: any, resolver: string, queryParams: any) => void;
	export const generateTestResolversData: () => Promise<TestData>;
	export const writeTestResolversDataToDB: (data: TestData, resolvers: UserResolvers) => Promise<void>;
	export const resolverTests: (testResolversData: TestData, resolvers: UserResolvers) => Promise<void>;
	export {};

	import { TestData } from '../../util/generator';
	import { TestCase } from './testResolvers';
	export const getQueryTests: (testResolversData: TestData) => TestCase[];

	import type { ApolloDatabaseUser, ApolloDatabaseDocument, ApolloDatabaseAnnotation, ApolloDatabaseAnnotationMember, ApolloDatabaseDocumentMember, ApolloDatabaseMention } from '../resolvers';
	type GeneratorOptions = {
	    xfdfString: string;
	};
	export type TestData = {
	    users: ApolloDatabaseUser[];
	    annotations: ApolloDatabaseAnnotation[];
	    documents: ApolloDatabaseDocument[];
	    annotationMembers: ApolloDatabaseAnnotationMember[];
	    documentMembers: ApolloDatabaseDocumentMember[];
	    mentions: ApolloDatabaseMention[];
	};
	class Generator {
	    private xfdfString;
	    data: TestData;
	    constructor(options: GeneratorOptions);
	    createUser(numberOfUser: number): Promise<void>;
	    createDocument(numberOfDoc: number, userId: string): Promise<void>;
	    createDocumentMember(userId: string, docId: string): Promise<void>;
	    createDocumentForAllUser(numberOfDoc: number): Promise<void>;
	    joinAllUserToAllDocument(): Promise<void>;
	    createAnnotation(numberOfAnnotation: number, docId: string, userId: string): Promise<void>;
	    createAnnotationMember(annotationId: string, docId: string, userId: string): Promise<void>;
	    createMention(annotationId: string, documentId: string, userId: string): Promise<void>;
	    createAnnotationForAllUser(numberOfAnnotation: number): Promise<void>;
	    export(): Promise<void>;
	    reset(): void;
	    getTimeStamp(): number;
	...
	import { TestData } from '../../util/generator';
	import { TestCase } from './testResolvers';
	export const getMutationTests: (testResolversData: TestData) => TestCase[];
